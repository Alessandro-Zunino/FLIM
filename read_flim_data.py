...
tau = h5py.File('08_Tau.h5')

dset_tau = np.asarray(tau["t0"]['channel0'])

# dset_tau[dset_tau>2] = 2
# dset_tau[dset_tau<0] = 0

flux = h5py.File('08_Intensity.h5')

dset_flux = np.asarray(flux["t0"]['channel0'])
...
# bounds_Int = {
#     'minInt' : np.min(dset_flux),
#     'maxInt' : np.max(dset_flux)
#     }

Image, Colorbar = cmap2D.showFLIM(dset_flux, dset_tau, bounds_Tau)
